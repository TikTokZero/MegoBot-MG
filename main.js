...
let prekey = []
let directorio = readdirSync("./Paplobot-MD")
...
const directories = ['./Paplobot-MD/', './jadibts/']
...
if (reason == 405) {
await fs.unlinkSync("./Paplobot-MD/" + "creds.json")
...
global.reloadHandler = async function(restatConn) {
  ...
  conn.bye = '*⦓          ⦓🏁⦔          ⦔*\n*🔄ꪶ→ @user*\n*ꪶ→ غادر المجموعه 💯*\n*⦓          ⦓🏁⦔          ⦔**';
  conn.spromote = '*[ 📌 ] @user اترقي بقي مسؤول🥳.*';
  conn.sdemote = '*[ 📌 ] @user اتخفضة رتبتة من مسؤول بقي عضو عادي🥲.*';
  conn.sDesc = '*[ 📌 ] وصف الروم اتغير✍️.*';
  conn.sSubject = '*[ 📌 ] اسم الروم اتغير✍️.*';
  conn.sIcon = '*[ 📌 ] صورة الروم اتغيرت📢.*';
  conn.sRevoke = '*[ 📌 ] رابط الروم اترستر🛠.*';
...
setInterval(async () => {
  ...
  const bio = `البوت شغال بواسطة بابلو 🎛:\n\n ${uptime}`;
